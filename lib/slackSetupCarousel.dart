...
class ImageCarousel extends StatefulWidget {
  ...
  final String?
  heroTagPrefix; // if provided, images will be wrapped in Hero widgets with tags `${heroTagPrefix}-$index`
  final ValueChanged<int?>?
  onImageTap; // optional callback when an image is tapped, receives the index
...
class _ImageCarouselState extends State<ImageCarousel> {
  ...
  Widget build(BuildContext context) {
    ...
    final hasRelative = widget.imagePaths.any(
      (p) => !(p.startsWith('/') || p.contains(':/')),
    );
    if (hasRelative) {
      return FutureBuilder<List<String>>(
        future: getIt<ISlackSetupRepository>().resolveImagePaths(
          widget.imagePaths,
        ),
  ...
  Widget _buildCarousel(BuildContext context, List<String> paths) {
    ...
    return Column(
      ...
      children: [
        ...
        SizedBox(height: widget.showSelected ? 8 : 0),
        widget.showSelected ? Row(
